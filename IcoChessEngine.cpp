...
{
	if (square < 0 || square > 63) throw std::out_of_range("lineAttacks called on out of range square.");

	bb64 occupied = pos.getOccupiedBB(),		slider = 1ULL << square,
		 horizontalMask = RANK[square / 8],		verticalMask = FILE[square % 8];

	bb64 horizontalAttacks = (((occupied & horizontalMask) - 2 * slider)
		^ bbReverse(bbReverse(occupied & horizontalMask) - 2 * bbReverse(slider))) & horizontalMask;

	bb64 verticalAttacks = (((occupied & verticalMask) - 2 * slider)
		^ bbReverse(bbReverse(occupied & verticalMask) - 2 * bbReverse(slider))) & verticalMask;
...
bb64 IcoChessEngine::diagonalAttacks(int square)
{
	...
	bb64 occupied = pos.getOccupiedBB(), slider = 1ULL << square,
		diagonalMask = DIAGONAL[((square / 8) - (square % 8)) & 15], antiDiagonalMask = ANTI_DIAGONAL[((square / 8) + (square % 8)) ^ 7];

	bb64 diagonalAttacks = (((occupied & diagonalMask) - 2 * slider)
							^ bbReverse(bbReverse(occupied & diagonalMask) - 2 * bbReverse(slider))) & diagonalMask;

	bb64 antiDiagonalAttacks = (((occupied & antiDiagonalMask) - 2 * slider)
							^ bbReverse(bbReverse(occupied & antiDiagonalMask) - 2 * bbReverse(slider))) & antiDiagonalMask;
