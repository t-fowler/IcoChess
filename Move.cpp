...
CandidateMoveList::CandidateMoveList(Move head) {
	moveList[0] = head;
	size = 1;
}

void CandidateMoveList::add(const Move newMove) {
...
bool CandidateMoveList::contains(const Move& other) {
	for (int i = 0; i < size; i++) {
		if (moveList[i].isEqual(other))
			return true;
	}
	return false;
}
